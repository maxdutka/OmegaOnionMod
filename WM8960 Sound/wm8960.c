		...
		unsigned int fmt)
{
	...
	default:
		dev_err(codec->dev, "wm8960 only supported as master\n");
		return -EINVAL;
	}
...
static int wm8960_configure_clocking(struct snd_soc_codec *codec)
{
	...
	if (wm8960->clk_id != WM8960_SYSCLK_PLL) {
			dev_err(codec->dev, "wm8960 only PLL mode is supported\n");
			return -EINVAL;
	}


	wm8960->freq_in = 24000000; // From fixed 24MHz generator
	...
	printk("wm8960 requested clocking BCLK: %d LRCLK: %d\n", bclk,lrclk);
		...
		for (j = 0; j < ARRAY_SIZE(dac_divs); ++j) {
			...
			for (k = 0; k < ARRAY_SIZE(bclk_divs); ++k) {
				if (sysclk == bclk * bclk_divs[k] / 10 &&
					...
					printk("wm8960 found valid PLL config at SYSCLK:%i\n", sysclk);
	...
	if (i == ARRAY_SIZE(sysclk_divs)) {
		dev_err(codec->dev, "wm8960 failed to configure PLL\n");
		return -EINVAL;
	}
		       ...
		       struct _pll_div *pll_div)
{
	...
	printk("wm8960 set PLL parms: N=%x K=%x pre_div=%d\n",
		...
		int source, unsigned int freq_in, unsigned int freq_out)
{
	...
	printk("wm8960 Set DAI PLL in:%i, out:%i\n", freq_in, freq_out);
